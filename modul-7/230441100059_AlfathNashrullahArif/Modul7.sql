...
-- 1
DELIMITER //
CREATE TRIGGER before_insert_nft
BEFORE INSERT ON NFT
FOR EACH ROW
BEGIN
	IF NEW.Harga < 0 THEN
	SIGNAL SQLSTATE '45000' 
	SET MESSAGE_TEXT = 'Harga NFT tidak boleh negatif';
	END IF;
END//
DELIMITER ;

INSERT INTO NFT (Nama_nft, Deskripsi, Id_kreator, Id_kategori, Harga) VALUES
('Straw ', 'Digital art kelompok bajak laut yonko topi jerami.', 7, 1, 6.5000);
('Akagam Pirates', 'Digital art kelompok bajak laut yonko rambut merah.', 4, 1, -4.5000);
('Straw Hat Pirates', 'Digital art kelompok bajak laut yonko topi jerami.', 6, 1, 4.5000);

-- 2 
DELIMITER //
CREATE TRIGGER before_update_pengguna
...
END//
DELIMITER ;
UPDATE Pengguna SET Username = '' WHERE Id_pengguna = 1;
UPDATE Pengguna SET Username = 'Coba' WHERE Id_pengguna = 1;
...
FOR EACH ROW
BEGIN
	IF EXISTS (SELECT 1 FROM NFT WHERE Id_kategori = OLD.Id_kategori) THEN
	SIGNAL SQLSTATE '45000' 
	SET MESSAGE_TEXT = 'Kategori sedang digunakan di tabel NFT dan tidak bisa dihapus';
	END IF;
END//
DELIMITER ;
...
-- 1 
DELIMITER //
CREATE TRIGGER after_insert_nft_to_favorit
AFTER INSERT ON NFT
FOR EACH ROW
BEGIN
	INSERT INTO Favorit (Id_pengguna, Id_nft)
	VALUES (NEW.Id_kreator, NEW.Id_nft)
	ON DUPLICATE KEY UPDATE Tanggal_ditambahkan = CURRENT_TIMESTAMP;
END//
DELIMITER ;

INSERT INTO NFT (Nama_nft, Deskripsi, Id_kreator, Id_kategori, Harga) VALUES
('Senja Digital', 'Ilustrasi matahari terbenam gaya vektor', 2, 1, 2.4000);
SELECT * FROM Favorit WHERE Id_pengguna = 2 AND Id_nft = (SELECT MAX(Id_nft) FROM NFT WHERE Id_kreator = 2);

-- 2 
DELIMITER //
CREATE TRIGGER update_status_nft_after_transaksi
AFTER UPDATE ON Transaksi
FOR EACH ROW
BEGIN
    IF NEW.Harga_beli > 0 AND OLD.Harga_beli != NEW.Harga_beli THEN
        UPDATE NFT
        SET Info_status = 'terjual'
        WHERE Id_nft = NEW.Id_nft AND Info_status != 'terjual';
    END IF;
END//
DELIMITER ;

UPDATE Transaksi SET Harga_beli = 8.0000 WHERE Id_transaksi = 5;
SELECT Id_nft, Nama_nft, Info_status FROM NFT WHERE Id_nft = 1;
SELECT * FROM NFT;

-- 3
DELIMITER //
CREATE TRIGGER after_delete_transaksi_nft_status
AFTER DELETE ON Transaksi
FOR EACH ROW
BEGIN
    UPDATE NFT
    SET Info_status = 'tersedia'
    WHERE Id_nft = OLD.Id_nft
    AND NOT EXISTS (
        SELECT 1 FROM Transaksi WHERE Id_nft = OLD.Id_nft
    );
END//
DELIMITER ;

DELETE FROM Transaksi WHERE Id_nft = 1;
SELECT Id_nft, Nama_nft, Info_status FROM NFT WHERE Id_nft = 1;
SELECT * FROM NFT;
