...
-- 1. Stored Procedure untuk Menambahkan UMKM Baru
DELIMITER //
...
-- 2. Stored Procedure untuk Memperbarui Nama Kategori UMKM
DELIMITER //
...
-- 3. Stored Procedure untuk Menghapus Pemilik UMKM
DELIMITER //
...
-- 4. Stored Procedure untuk Menambahkan Produk Baru
DELIMITER //
...
-- 5. Stored Procedure untuk Mengambil Data UMKM Berdasarkan ID
DELIMITER //
...
CALL AddUMKM('Bakso Enak Jaya', 12);

-- 2. Memperbarui Nama Kategori UMKM
CALL UpdateKategoriUMKM(1, 'Kuliner Modern');

-- 3. Menghapus Pemilik UMKM
DELETE FROM umkm WHERE id_pemilik = 3;
CALL DeletePemilikUMKM(3);

-- 4. Menambahkan Produk Baru
CALL AddProduk(1, 'Bakso Jumbo', 15000.00);

-- 5. Mengambil Data UMKM Berdasarkan ID
SET @nama_usaha = '';
SET @jumlah_karyawan = 0;
CALL GetUMKMByID(1, @nama_usaha, @jumlah_karyawan);
SELECT @nama_usaha AS 'Nama Usaha', @jumlah_karyawan AS 'Jumlah Karyawan';
