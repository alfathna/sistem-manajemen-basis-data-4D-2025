...
CALL AddProduk(1, 'Kopi Arabika Premium', 25000.00);
SELECT id_produk, id_umkm, nama_produk, harga FROM produk_umkm WHERE id_umkm = 1;

-- 5
DELIMITER //
CREATE PROCEDURE GetUMKMByID_Fields(
IN p_id_umkm INT,
...
BEGIN
SELECT nama_usaha, alamat_usaha, tahun_berdiri
INTO p_nama_usaha, p_alamat_usaha, p_tahun_berdiri
FROM umkm
...
CALL GetUMKMByID_Fields(2, @nama, @alamat, @tahun);
SELECT @nama AS Nama_Usaha, @alamat AS Alamat_Usaha, @tahun AS Tahun_Berdiri;
