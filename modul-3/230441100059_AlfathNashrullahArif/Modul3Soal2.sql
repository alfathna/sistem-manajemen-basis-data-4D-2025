...
DROP PROCEDURE UpdateMaster;
CALL UpdateMaster(1, 'Digital Art');
SELECT * FROM Kategori;
...
OUT out_tanggal DATE
)
BEGIN
SELECT Username, Email, Tanggal_daftar
INTO out_username, out_email, out_tanggal
FROM Pengguna
WHERE Id_pengguna = in_id;
END //

DELIMITER ;

CALL GetDataMasterByID(2, @username, @email, @tanggal);
SELECT @username AS Username, @email AS Email, @tanggal AS TanggalDaftar;
...
CREATE PROCEDURE UpdateFieldTransaksi(
    IN id INT,
    INOUT p_harga_beli DECIMAL(10,2),
    INOUT p_tanggal_beli DATETIME
)
BEGIN
    UPDATE Transaksi
    SET 
        Harga_beli = IFNULL(p_harga_beli, Harga_beli),
        Tanggal_transaksi = IFNULL(p_tanggal_beli, Tanggal_transaksi)
    WHERE Id_transaksi = id;
END//

DELIMITER ;

SET @p_harga_beli = 5.5000, @p_tanggal_beli = NULL;
CALL UpdateFieldTransaksi(1, @p_harga_beli, @p_tanggal_beli);
...
BEGIN
    -- Hapus dari tabel Favorit (jika ada referensi ke pengguna)
    DELETE FROM Favorit WHERE Id_pengguna = in_id;

    -- Hapus dari tabel Transaksi (baik sebagai pembeli maupun kreator)
    DELETE FROM Transaksi WHERE Id_kreator = in_id OR Id_pembeli = in_id;

    -- Hapus dari tabel NFT yang dibuat oleh pengguna tersebut
    DELETE FROM NFT WHERE Id_kreator = in_id;

    -- Terakhir, hapus dari tabel Pengguna
    DELETE FROM Pengguna WHERE Id_pengguna = in_id;
END //

DELIMITER ;

CALL DeleteEntriesByIDMaster(4);
SELECT * FROM Pengguna WHERE Id_pengguna = 4;
SELECT * FROM Transaksi WHERE Id_kreator = 4 OR Id_pembeli = 4;
SELECT * FROM NFT WHERE Id_kreator = 4;
SELECT * FROM Favorit WHERE Id_pengguna = 4;
