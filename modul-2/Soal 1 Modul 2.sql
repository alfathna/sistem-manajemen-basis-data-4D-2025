...
CREATE DATABASE db_umkm_Harryjaya;
...
CREATE TABLE skala_umkm (
    id_skala INT AUTO_INCREMENT NOT NULL PRIMARY KEY,
...
CREATE TABLE kabupaten_kota (
    id_kabupaten_kota INT AUTO_INCREMENT NOT NULL PRIMARY KEY,
    nama_kabupaten_kota VARCHAR(100) NOT NULL
);

CREATE TABLE pemilik_umkm (
    id_pemilik INT AUTO_INCREMENT NOT NULL PRIMARY KEY,
...
ALTER TABLE umkm ADD CONSTRAINT fkid_pemilik FOREIGN KEY (id_pemilik) REFERENCES pemilik_umkm(id_pemilik)
ALTER TABLE umkm ADD CONSTRAINT fkid_kategori FOREIGN KEY (id_kategori) REFERENCES kategori_umkm(id_kategori)
ALTER TABLE umkm ADD CONSTRAINT fkid_skala FOREIGN KEY (id_skala) REFERENCES skala_umkm(id_skala)
ALTER TABLE umkm ADD CONSTRAINT fkid_kabupaten_kota FOREIGN KEY (id_kabupaten_kota) REFERENCES kabupaten_kota(id_kabupaten_kota)
...
ALTER TABLE produk_umkm ADD CONSTRAINT fkid_umkm FOREIGN KEY (id_umkm) REFERENCES umkm(id_umkm)
...
SELECT 
    ...
    kab.nama_kabupaten_kota AS 'Kabupaten / Kota', 
...
SELECT 
    ...
    u.tahun_berdiri
FROM db_umkm_harryjaya.umkm u
JOIN db_umkm_harryjaya.pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN db_umkm_harryjaya.kategori_umkm k ON u.id_kategori = k.id_kategori
JOIN db_umkm_harryjaya.skala_umkm s ON u.id_skala = s.id_skala
JOIN db_umkm_harryjaya.kabupaten_kota kk ON u.id_kabupaten_kota = kk.id_kabupaten_kota;

select * from view_umkm_detail;
===========================================================================

-- Soal 2
DROP VIEW IF EXISTS view_pemilik_dan_usaha;
...
SELECT 
    ...
    p.email,
    u.nama_usaha
FROM db_umkm_harryjaya.pemilik_umkm p
JOIN db_umkm_harryjaya.umkm u ON p.id_pemilik = u.id_pemilik;
...
-- Soal 3
DROP VIEW IF EXISTS view_produk_umkm;
...
SELECT 
    ...
    pr.deskripsi_produk,
    pr.harga
FROM db_umkm_harryjaya.produk_umkm pr
JOIN db_umkm_harryjaya.umkm u ON pr.id_umkm = u.id_umkm;
...
-- Soal 4
DROP VIEW IF EXISTS view_umkm_menengah;
...
-- Soal 5
DROP VIEW IF EXISTS view_umkm_per_kota;
...
SELECT * FROM view_umkm_per_kota ORDER BY jumlah_umkm desc;
===========================================================================