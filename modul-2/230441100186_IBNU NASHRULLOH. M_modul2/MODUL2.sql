...
GROUP BY kk.id_kabupaten_kota, kk.nama_kabupaten_kota;
...
-- ================================
-- 3. ADD FOREIGN KEY (ALTER TABLE)
-- ================================

ALTER TABLE Jadwal_Film
    ADD CONSTRAINT fk_jadwal_film FOREIGN KEY (kode_film) REFERENCES Film(kode_film) ON DELETE CASCADE,
    ADD CONSTRAINT fk_jadwal_studio FOREIGN KEY (id_studio) REFERENCES Studio(id_studio) ON DELETE CASCADE;

ALTER TABLE Tiket
    ADD CONSTRAINT fk_tiket_pelanggan FOREIGN KEY (id_pelanggan) REFERENCES Pelanggan(id_pelanggan) ON DELETE CASCADE,
    ADD CONSTRAINT fk_tiket_jadwal FOREIGN KEY (id_jadwal) REFERENCES Jadwal_Film(id_jadwal) ON DELETE CASCADE;

ALTER TABLE Pembayaran
    ADD CONSTRAINT fk_pembayaran_tiket FOREIGN KEY (id_tiket) REFERENCES Tiket(id_tiket) ON DELETE CASCADE;

-- ================================
-- 4. MASUKKAN DATA (Urutannya penting!)
-- ================================
...
-- Cek id_tiket yang dihasilkan sebelum insert ke Pembayaran:
-- SELECT * FROM Tiket;

-- Pembayaran
-- Asumsikan id_tiket adalah 1 dan 2 dari hasil insert sebelumnya
INSERT INTO Pembayaran (id_tiket, metode_pembayaran, tanggal_pembayaran, jumlah_bayar) VALUES
(1, 'E-Wallet', '2024-03-25', 100000),
(2, 'Kartu Kredit', '2024-03-26', 50000);
...
-- jawaban soal 2
-- no 1
CREATE VIEW view_tiket_dan_pelanggan AS
SELECT 
    t.id_tiket,
    p.nama,
    p.email,
...
-- no 3
CREATE VIEW view_jadwal_rating_r AS
SELECT 
    f.judul,
    f.rating,
    j.tanggal,
    j.jam_tayang
FROM Film f
JOIN Jadwal_Film j ON f.kode_film = j.kode_film
WHERE f.rating = 'R';

-- no 4
CREATE VIEW view_total_tiket_per_film AS
SELECT 
    f.judul,
    SUM(t.jumlah) AS total_tiket_terjual
FROM Tiket t
JOIN Jadwal_Film j ON t.id_jadwal = j.id_jadwal
...
SELECT 
    ...
    py.jumlah_bayar
FROM Pembayaran py
JOIN Tiket t ON py.id_tiket = t.id_tiket
JOIN Pelanggan p ON t.id_pelanggan = p.id_pelanggan;
