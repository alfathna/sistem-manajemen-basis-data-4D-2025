...
-- -- -- -- -- -- -- --
-- Soal 1 --
CREATE VIEW view_nft_kategori AS
SELECT 
n.Id_nft,
n.Nama_nft,
n.Deskripsi,
...
t.Id_transaksi,
n.Nama_nft,
p.Username AS Nama_pembeli,
t.Harga_beli,
t.Tanggal_transaksi
FROM Transaksi t
JOIN NFT n ON t.Id_nft = n.Id_nft
JOIN Pengguna p ON t.Id_pembeli = p.Id_pengguna;
SELECT * FROM view_transaksi_lengkap;

-- Soal 3 --
CREATE VIEW view_nft_terjual_dengan_kreator AS
SELECT 
n.Id_nft,
n.Nama_nft,
n.Harga,
n.Info_status,
p.Username AS Nama_kreator
FROM NFT n
JOIN Pengguna p ON n.Id_kreator = p.Id_pengguna
WHERE n.Info_status = 'terjual';
SELECT * FROM view_nft_terjual_dengan_kreator;

-- Soal 4 --
CREATE VIEW view_total_pendapatan_kreator AS
SELECT 
p.Username AS Nama_kreator,
SUM(t.Harga_beli) AS Total_pendapatan
FROM Transaksi t
JOIN Pengguna p ON t.Id_kreator = p.Id_pengguna
GROUP BY p.Username;
SELECT * FROM view_total_pendapatan_kreator;

-- Soal 5 --
CREATE VIEW view_nft_favorit_pengguna AS
SELECT 
f.Id_favorit,
u.Username,
n.Nama_nft,
...
FROM Favorit f
JOIN Pengguna u ON f.Id_pengguna = u.Id_pengguna
JOIN NFT n ON f.Id_nft = n.Id_nft;
SELECT * FROM view_nft_favorit_pengguna;
