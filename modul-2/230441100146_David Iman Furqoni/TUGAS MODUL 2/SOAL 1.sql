...
SELECT	P.nik AS 'NIK', 
	p.nama_lengkap AS 'Nama Lengkap Pemilik',
	p.jenis_kelamin AS 'Jenis Kelamin',
	p.nomor_telepon AS 'Nomor Telepon',
	p.email AS 'Email',
	u.nama_usaha AS 'Usaha yang Dimiliki'
FROM pemilik_umkm p
...
SELECT 
    k.nama_kabupaten_kota AS `Kabupaten / Kota`,
...
SELECT * FROM view_umkm_per_kota ORDER BY `Jumlah UMKM` DESC;
